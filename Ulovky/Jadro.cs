...
namespace Ulovky
{
    public class Jadro
    {
                    ...
                    using (SQLiteCommand mycommand = new SQLiteCommand(sql, cnn))
                    {
                        using (SQLiteDataReader reader = mycommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                list.Add(new Ulovok(datum, cisloReviru,nazovReviru,lokalita,druh,dlzka,vaha,sposobLovu,nastraha,pustena,poznamky,flagPoznamky));
                            }
                        }
                    }
                }
        ...
        {
            var listRokov = new List<string>(ListRokov.Where(x => x.Key == User).Select(x => x.Value).OrderBy(x =>x).ToList());

            var predchadzajuci = listRokov.First() != Rok.ToString() ? listRokov[listRokov.IndexOf(Rok.ToString()) - 1] : listRokov.Last();
            var nasledujuci = listRokov.Last() != Rok.ToString() ? listRokov[listRokov.IndexOf(Rok.ToString()) + 1] : listRokov.First();
        ...
        public ListViewItem[] KoncorocnaTabulka()
        {
            ...
            var druhyRyby = GetStringField(SqlDotazy.SqlDotazy.GetSumarneQuery(Rok.ToString(CultureInfo.InvariantCulture))).ToList();
            var pocetUlovkov = new List<int>();
            var vahaUlovkov = new List<int>();

            for (int i = 1; i <= druhyRyby.Count; i++)
            {
                String sqlPocet = "select count(*) from ulovky where rok='" + Rok + "' and user='" + User + "' and druh='" + druhyRyby[i-1] + "'";
                String sqlVaha = "select sum(vaha) from ulovky where rok='" + Rok + "' and user='" + User + "' and druh='" + druhyRyby[i-1] + "'";
                ...
                list.Add(new SumarnaTabulka.SumarnaTabulka(Rok, i.ToString(), druhyRyby[i - 1], vahaUlovkov[i - 1], pocetUlovkov[i - 1]));
            }
